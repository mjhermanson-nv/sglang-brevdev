...
def _(port, print_highlight):
    ...
    _response = client.chat.completions.create(model='qwen/qwen2.5-0.5b-instruct', messages=[{'role': 'user', 'content': 'List 3 countries and their capitals.'}], temperature=0, max_tokens=64)
...
def _(client, print_highlight):
    ...
    _response = client.chat.completions.create(model='qwen/qwen2.5-0.5b-instruct', messages=[{'role': 'user', 'content': 'Complete this sentence: The weather today is'}], temperature=0.7, max_tokens=20, logit_bias={'12345': 50, '67890': -50, '11111': 25})
    print_highlight(f'Response with logit bias: {_response.choices[0].message.content}')  # Increase likelihood of token ID 12345  # Decrease likelihood of token ID 67890  # Slightly increase likelihood of token ID 11111
    return
...
@app.cell
def _(client, print_highlight):
    _response = client.chat.completions.create(model='qwen/qwen2.5-0.5b-instruct', messages=[{'role': 'system', 'content': 'You are a knowledgeable historian who provides concise responses.'}, {'role': 'user', 'content': 'Tell me about ancient Rome'}, {'role': 'assistant', 'content': 'Ancient Rome was a civilization centered in Italy.'}, {'role': 'user', 'content': 'What were their major achievements?'}], temperature=0.3, max_tokens=128, top_p=0.95, presence_penalty=0.2, frequency_penalty=0.2, n=1, seed=42)
    print_highlight(_response.choices[0].message.content)  # Lower temperature for more focused responses  # Reasonable length for a concise response  # Slightly higher for better fluency  # Mild penalty to avoid repetition  # Mild penalty for more natural language  # Single response is usually more stable  # Keep for reproducibility
    return
...
def _(client, print_highlight):
    ...
    # Note: You need to get the actual token IDs from your tokenizer
    # For demonstration, we'll use some example token IDs
    _response = client.completions.create(model='qwen/qwen2.5-0.5b-instruct', prompt='The best programming language for AI is', temperature=0.7, max_tokens=20, logit_bias={'12345': 75, '67890': -100, '11111': -25})
    print_highlight(f'Response with logit bias: {_response.choices[0].text}')  # Strongly favor token ID 12345  # Completely avoid token ID 67890  # Slightly discourage token ID 11111
    return
...
@app.cell
def _(client, print_highlight):
    _response = client.completions.create(model='qwen/qwen2.5-0.5b-instruct', prompt='List 3 countries and their capitals.', temperature=0, max_tokens=64, n=1, stop=None)
    print_highlight(f'Response: {_response}')
    return
...
@app.cell
def _(client, print_highlight):
    _response = client.completions.create(model='qwen/qwen2.5-0.5b-instruct', prompt='Write a short story about a space explorer.', temperature=0.7, max_tokens=150, top_p=0.9, stop=['\n\n', 'THE END'], presence_penalty=0.3, frequency_penalty=0.3, n=1, seed=123)
    print_highlight(f'Response: {_response}')  # Moderate temperature for creative writing  # Longer response for a story  # Balanced diversity in word choice  # Multiple stop sequences  # Encourage novel elements  # Reduce repetitive phrases  # Generate one completion  # For reproducible results
    return
